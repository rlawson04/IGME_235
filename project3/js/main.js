...
function createLabelsAndButtons(){
    ...
    let  startLabel1 = new PIXI.Text("Deep Space");
    ...
    let startLabel2 = new PIXI.Text("Highscores!");
    ...
    let startButton = new PIXI.Text("New Game");
    ...
    startButton.on('pointerout', e => e.current.target.alpha = 1.0); // ditto
    ...
    gameOverText.x = sceneWidth/2 - 100;
...
function gameLoop(){
    ...
    // lerp (linearinterpolate) the x & y values with lerp()
...
function end(){
    paused = true;
    // clear out level
    circles.forEach(c=>gameScene.removeChild(c)); // arrow function
    circles = [];
...
function loadSpriteSheet(){
    ...
    let spriteSheet = PIXI.BaseTexture.from("images/galaga.png");   
    let numFrames = 5;
    let textures = [];
    for (let i = 0; i < numFrames; i++){
        let frame = new PIXI.Texture(spriteSheet, new PIXI.Rectangle(i*34 + 290, 1, 30, 30));
        textures.push(frame);
    }
    return textures;
}
function createExplosion(x, y, frameWidth, frameHeight){
    ...
    expl.x = x - w2 + 10; // make explosion appear in the center of the circle
    expl.y = y - h2 - 10;
    expl.animationSpeed = 1/5;
    expl.loop = false;
